...
impl RefFromJsValue for KordScaleCandidate {
    fn ref_from_js_value(abi: &JsValue) -> JsRes<<KordScaleCandidate as RefFromWasmAbi>::Anchor>
    where
        Self: Sized + RefFromWasmAbi,
    {
        let ptr_value = Reflect::get(abi, &JsValue::from_str("ptr"))?;
        ...
        let object = abi.dyn_ref::<Object>().ok_or("Value is not an object.")?;
        if object.constructor().name() != "KordScaleCandidate" {
            return Err("Invalid object type.".into());
        }
        ...
        // We have confirmed that the JsValue is, indeed, an Object, and that
        // it is of the proper type.
        let value = unsafe { KordScaleCandidate::ref_from_abi(ptr) };
...
mod tests {
    ...
    fn test_scale_candidates_basic() {
        ...
        let first_candidate = KordScaleCandidate::ref_from_js_value(&first).unwrap().clone();
    ...
    fn test_scale_candidates_minor_chord() {
        ...
        for i in 0..candidates.length() {
            ...
            let candidate = KordScaleCandidate::ref_from_js_value(&candidate_val).unwrap().clone();
    ...
    fn test_scale_candidates_with_different_roots() {
        ...
        let c_first = KordScaleCandidate::ref_from_js_value(&c_candidates.get(0)).unwrap().clone();
        let g_first = KordScaleCandidate::ref_from_js_value(&g_candidates.get(0)).unwrap().clone();
