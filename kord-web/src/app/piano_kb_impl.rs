...
use klib::core::{base::{Parsable, HasName}, note::Note};
use thaw_utils::ArcOneCallback;
use crate::app::piano::setup_note_highlight;
...
pub fn setup_keyboard_listeners(midi_player: Rc<MidiPlayer>, on_key_press: ArcOneCallback<Note>, pressed_notes: RwSignal<HashSet<String>>) {
    ...
    add("a", "C4", &mut map); add("s", "D4", &mut map); add("d", "E4", &mut map); add("f", "F4", &mut map);
    add("g", "G4", &mut map); add("h", "A4", &mut map); add("j", "B4", &mut map); add("k", "C5", &mut map);
    add("w", "C#4", &mut map); add("e", "D#4", &mut map); add("t", "F#4", &mut map); add("y", "G#4", &mut map); add("u", "A#4", &mut map);
...

    {
        ...
        let cleanup = use_event_listener(leptos::prelude::window(), leptos::ev::keydown, move |ev: KeyboardEvent| {
            ...
            if let Some(&note) = map.get(&key) {
                ...
                if !set.insert(key.clone()) { return; }
